...
private:
	struct HDRPixel {
		...
		HDRPixel(const Color &color) {
			...
			r_ = (unsigned char)(color.x * d);
			g_ = (unsigned char)(color.y * d);
			b_ = (unsigned char)(color.z * d);
...
public:
	...
	inline Color& image(const int x, const int y) {
		return image_[y * width_ + x];
	}
	...
	void load_unsafe(const std::string &filename) {
		...
		for (int y = 0; y < height_; ++y) {
			for (int x = 0; x < width_; ++x) {
				...
				image_[(height_ - 1 - y) * width_ + x].x = tmp_data[(y * width_ + x) * 4 + 0] * pow(2, e - 128.0f) / 256.0f;
				image_[(height_ - 1 - y) * width_ + x].y = tmp_data[(y * width_ + x) * 4 + 1] * pow(2, e - 128.0f) / 256.0f;
				image_[(height_ - 1 - y) * width_ + x].z = tmp_data[(y * width_ + x) * 4 + 2] * pow(2, e - 128.0f) / 256.0f;
			}
		}
	}
