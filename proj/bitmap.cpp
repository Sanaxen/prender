...
Image *BitMap::Read_Bmp(char *filename)
{
	...
	if(color != 24){
		...
		printf("Error: %s is not 24bit color image[%d]\n", filename, color);
		return NULL;
	}
...
/*
A. Buades, B. Coll, J.M. Morel ÅgA non local algorithm for image denoisingÅh
IEEE Computer Vision and Pattern Recognition 2005, Vol 2, pp: 60-65, 2005
*/
#define max(a,  b)	(a >= b)? a : b
#define min(a,  b)	(a <= b)? a : b

float* sample_template(int width, int height, float* src, int kKernel, int x, int y)
{
	const int kHalfKernel = kKernel / 2;

	float* t = new float[3 * kKernel*kKernel];
	memset(t, '\0', sizeof(float) * 3 * kKernel*kKernel);
...
float distance_sqared_template(int width, int height, const float* a, const float* b, int kKernel)
{
	float accum = 0.0f;
	for (int i = 0; i < 3 * kKernel*kKernel; ++i) {
		accum += pow(a[i] - b[i], 2.f);
	}
	return accum;
};
...
{
	param_h = max(0.0001f, param_h);
	sigma = max(0.0001f, sigma);

	const int kKernel = 3;// 5;
	const int kSupport = 13;// 13;
	const int kHalfKernel = kKernel / 2;
...
#pragma omp parallel for
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; ++x) 
		{
			...
			int focus_size;
			float* focus = sample_template(width, height, src, kKernel, x, y);

			float sum[3] = { 0, 0, 0 };
			float sum_weight = 0.0;
			for (int sx = x - kHalfSupport; sx <= x + kHalfSupport; ++sx) {
				for (int sy = y - kHalfSupport; sy <= y + kHalfSupport; ++sy) {
					float* target = sample_template(width, height, src, kKernel, sx, sy);
					float dist = distance_sqared_template(width, height, focus, target, kKernel);
					float arg = -max(dist - 2.0f * sigma * sigma, 0.0f) / (param_h * param_h);
					float weight = expf(arg);
			...
			delete[] focus;
			float color[3] = { sum[0] / sum_weight, sum[1] / sum_weight, sum[2] / sum_weight };
	...
	memcpy(src, dst, 3 * width*height*sizeof(float));
	delete[] dst;
