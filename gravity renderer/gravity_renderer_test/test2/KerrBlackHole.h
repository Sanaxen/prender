...
public:
	...
	double y;
	double z;
	KerrBlackHole(double x_, double y_, double z_, double rDisc_, double m, double a_, Spherical& cameraPos)
	{
		rDisc = rDisc_;
		x = x_;
		y = y_;
	...
	{
		return Vector3d(p.x - x, p.y - y, p.z - z);
	}
	...
	int RayToOrdinaryDifferentialEquationInitial(KerrBlackHoleWrkParam& prm, Vector3d& dir, double* y, double* dydx)
	{
		Vector3d nrm = normalize(Spherical(r0, theta0, phi0).ToVector());
		Vector3d X, Y;
		...
		double y0 = dot(X, dir)*-1;
		double x0 = dot(Y, dir)*-1;

		double rdot0 = cos(y0)*cos(x0);
		double thetadot0 = sin(y0) / r0;

		dydx[0] = rdot0;
		dydx[1] = thetadot0;
		dydx[2] = cos(y0)*sin(x0) / (r0*sin(theta0));

		initial2(prm, y, dydx, dir);
		return 0;
