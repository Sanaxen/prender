...
namespace GraviRayTraceSharp.Scene
{
    ...
    public class DefaultPathSceneGenerator : ISceneGenerator
    {
        public SceneDescription GetScene(int frame, double fps)
        {
            ...
            double calmFactor = 0.0;// Math.Pow((600 - r) / 575, 20); 
            ...
            result.CameraAperture = r;// 24.00 / 500.0 * r + 3.2;
            result.CameraTilt = 0.0;// 8.0 * Math.Cos(phi * Math.PI / 180) * (1 - calmFactor);
            result.CameraYaw = 0.0; //calmFactor * 1.0; // we will be 'landing' on the accretion disc...
